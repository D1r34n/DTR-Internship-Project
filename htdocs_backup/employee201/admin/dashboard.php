        ...
        if ($res) {
            while ($r = $res->fetch_assoc()) {
                $rows[] = [
                    ...
                    'department' => $r['department'] ?? '',
                    'position'   => $r['position'] ?? '',
                    'status'     => $r['date_of_separation'] ? date('Y-m-d', strtotime($r['date_of_separation'])) : '',
                ];
            }
        }
    } else {
...
/* OFFBOARDED (TOTAL HISTORICAL) — not month-only */
...
const statusColHead = document.getElementById('statusColHead');
...
function openListModal(kind){
  listTitleEl.textContent   = kind === 'new' ? 'New Employees (This Month)' : 'Terminated Employees (This Month)';
  dateColHeader.textContent = kind === 'new' ? 'Date Hired' : 'Date Terminated';
  statusColHead.style.display = kind === 'terminated' ? '' : 'none';
...
function closeListModal(){ listBackdrop.classList.remove('show'); }

document.getElementById('openNewModal').addEventListener('click', ()=>openListModal('new'));
document.getElementById('openTerminatedModal').addEventListener('click', ()=>openListModal('terminated'));
listCloseBtn.addEventListener('click', closeListModal);
listBackdrop.addEventListener('click', (e)=>{ if (e.target === listBackdrop) closeListModal(); });
