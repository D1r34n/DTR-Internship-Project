...
if ($userMessage === "system info") {
    $systemInfo  = "📊 You are using the Employee 201 Management System.";
    $systemInfo .= "\nCurrent AI provider: " . strtoupper($AI_PROVIDER);
    $systemInfo .= "\n\n👉 <a href='about.php' class='chat-link'>Go to About Page</a>";
...
if ($userMessage === "birthday today") {
    ...
    if ($res && $res->num_rows > 0) {
        ...
        send("🎂 Birthdays today:\n" . implode("\n", $list));
    } else {
        send("🎂 No employees have birthdays today.");
    }
}

if ($userMessage === "birthdays this month") {
    $res = $conn->query("SELECT id, first_name, last_name, birthdate 
                         FROM employees 
                         WHERE MONTH(birthdate) = MONTH(CURDATE()) 
                         ORDER BY DAY(birthdate)");
    $list = [];
    while ($row = $res->fetch_assoc()) {
        $list[] = "#{$row['id']} {$row['first_name']} {$row['last_name']} ({$row['birthdate']})";
    }
    send($list ? "🎉 Birthdays this month:\n" . implode("\n", $list) : "No birthdays this month.");
}

if ($userMessage === "count by department") {
    $res = $conn->query("SELECT department, COUNT(*) AS c FROM employees GROUP BY department");
    $list = [];
    while ($row = $res->fetch_assoc()) {
        $list[] = "{$row['department']}: {$row['c']}";
    }
    send("🏢 Employee count by department:\n" . implode("\n", $list));
}

if ($userMessage === "latest admins") {
    $res = $conn->query("SELECT username, date_added FROM admins ORDER BY date_added DESC LIMIT 5");
    $list = [];
    while ($row = $res->fetch_assoc()) {
        $list[] = "{$row['username']} (Created: {$row['date_added']})";
    }
    send("🛡️ Latest Admin Accounts:\n" . implode("\n", $list));
}
...
if (preg_match('/^get employee (\w+)/', $userMessage, $m)) {
    $code = $conn->real_escape_string($m[1]);
    $res = $conn->query("SELECT id, employee_code, first_name, last_name, position, department 
                         FROM employees 
    ...
    if ($res && $res->num_rows > 0) {
        ...
        send("👤 Employee {$emp['employee_code']}: {$emp['first_name']} {$emp['last_name']}, Position: {$emp['position']}, Department: {$emp['department']}");
    } else {
        send("❌ Employee not found with code $code.");
    }
}
...
if ($userMessage === "clear chat") {
    unset($_SESSION['chat']);  // remove only chat history
...
if ($userMessage === "help" || $userMessage === "commands") {
    $commands = [
        "system info", "show my role", 
        "count employees", "count male", "count female", "count admins", 
        "hired this month", "terminated this month", 
        "get employee {id}", "search employee {name}", 
        "list employees", "latest employees",
        "birthday today", "birthdays this month",
        "show latest logs", "recent activities",
        "count by department", "latest admins",
        "open dashboard", "open manage", "open add",
        "logout", "clear chat"
    ];
...
if ($userMessage === "thank you" || $userMessage === "thanks") {
    send("🙏 You're welcome! Happy to help.");
}
