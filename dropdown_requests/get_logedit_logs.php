...
$stmt = $pdo->prepare("
    SELECT
        ...
        CASE WHEN EXISTS (
            SELECT 1
            FROM log_edit_requests ler1
            WHERE ler1.log_id = l.id
              AND ler1.status = 'pending'
        ) THEN 1 ELSE 0 END AS has_pending,

        -- approved edit exists (FINALIZED edit)
        CASE WHEN EXISTS (
            SELECT 1
            FROM log_edit_requests ler2
            WHERE ler2.log_id = l.id
              AND ler2.status = 'approved'
